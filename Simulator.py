...
class Mirror: #A mirror that reflects all radiation. It does not emit or absorb radiation.
    ...
    def draw(self): #Draw the mirror as a gray rectangle.
        ...
        pygame.draw.rect(self.simulation.screen, color, ((self.simulation.slots.index(self) + 1)*500/(len(self.simulation.slots) + 1), 10, 10, 150)) #Draw a rectangle at (x, y) with width and height of 10 pixels.
        img = font.render(f"Mirror", True, color) #Render the temperature text.
        self.simulation.screen.blit(img, ((self.simulation.slots.index(self) + 1)*500/(len(self.simulation.slots) + 1) - 50, 200)) #Draw the temperature text next to the blackbody.
...
class HeatSource:
    ...
    def draw(self): #Draw the heat source as a rectangle. Color depends on temperature.
        ...
        self.simulation.screen.blit(img, ((self.simulation.slots.index(self) + 1)*500/(len(self.simulation.slots) + 1) - 50, 220)) #Draw the temperature text next to the blackbody.
    ...
    def emit_radiation(self): #Mirrors do not emit radiation.
        ...
        if index - 1 < 0: #If there is no object to the left...
            ...
            if isinstance(self.simulation.slots[index - 1], Mirror): #If the object to the left is a mirror...
                ...
                self.simulation.slots[index - 1].incoming_radiation_left += emission #Send radiation to the previous (left) object in the list.
        if  index + 1 > len(self.simulation.slots) - 1: #If there is no object to the right...
            ...
            if isinstance(self.simulation.slots[index + 1], Mirror): #If the object to the right is a mirror...
                ...
                self.simulation.slots[index + 1].incoming_radiation_right += emission #Send radiation to the next (right) object in the list.
...
class Simulation: #This is the main class. It contains all the code for running the simulation.
    ...
    def __init__(self): #Constructor for the Simulation object. This code gets run whenever we make a new Simulation object, like: Simulation(). 
            self.screen = pygame.display.set_mode((500, 400)) #Sets the size of the display in terms of number of pixels. Width, then height.
